...
public class SPARouting {
    ...
    RouterFunction<ServerResponse> spaRouter(SPAController controller) {
        ...
        RequestPredicate spaPredicate = path("/api/**").or(path("/v3/api-docs")).or(path("/v3/api-docs/**")).or(path("/internal/**")).or(path("/style/**")).or(path("/sitemap/**")).or(path("/error")).or(path("/assets/**")).or(pathExtension(extensions::contains)).negate();
