...
import eu.ebrains.kg.projects.ebrains.source.DatasetV3;
import eu.ebrains.kg.projects.ebrains.target.Dataset;
...
public class DatasetTranslator extends EBRAINSTranslator<DatasetV3, Dataset, DatasetTranslator.Result> {

    private static final String QUERY_ID = "1967ced9-e3f9-4d8f-a2e6-296f3fb6329f";

    public static class Result extends ResultsOfKG<DatasetV3> {
    }
    ...
    public Dataset translate(DatasetV3 dataset, DataStage dataStage, boolean liveMode, TranslatorUtils translatorUtils) throws TranslationException {
        if (!CollectionUtils.isEmpty(dataset.getVersions()) && dataset.getVersions().size() > 1) {
            ...
            List<eu.ebrains.kg.projects.ebrains.source.commons.Version> sortedVersions = EBRAINSTranslatorUtils.sort(dataset.getVersions(), translatorUtils.getErrors());
