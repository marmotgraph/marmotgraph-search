...
import eu.ebrains.kg.projects.ebrains.translators.utils.SchemaOrgConverter;
import eu.ebrains.kg.projects.ebrains.translators.utils.SpecimenResolver;
import eu.ebrains.kg.projects.ebrains.translators.utils.SpecimenTranslator;
...
public class DatasetVersionTranslator extends EBRAINSTranslator<DatasetVersionV3, DatasetVersion, DatasetVersionTranslator.Result> {
    ...
    public Map<String, Object> populateTranslationContext(ESServiceClient esServiceClient, ESHelper esHelper, DataStage stage) {
        return Collections.singletonMap(SPECIMEN_LOOKUP_MAP, new SpecimenResolver(esServiceClient, esHelper).loadSpecimenLookupMap(stage));
    }
    ...
    public static class Result extends ResultsOfKG<DatasetVersionV3> {
    }
    ...
    public DatasetVersion translate(DatasetVersionV3 datasetVersion, DataStage dataStage, boolean liveMode, TranslatorUtils translatorUtils) throws TranslationException {
        ...
        if (accessibility != null) {
            switch (accessibility) {
                case CONTROLLED_ACCESS:
                    ...
                    break;
                case UNDER_EMBARGO:
                    Value<String> embargoMessage = value(EBRAINSTranslatorUtils.createEmbargoMessage("dataset", datasetVersion.getFileRepository(), uuid));
        ...
        if (!CollectionUtils.isEmpty(custodians)) {
            d.setCustodians(custodians.stream().map(c -> new TargetInternalReference(IdUtils.getUUID(c.getId()), EBRAINSTranslatorUtils.getFullName(c.getFullName(), c.getFamilyName(), c.getGivenName()))).collect(Collectors.toList()));
        }

        if (!CollectionUtils.isEmpty(datasetVersion.getRelatedPublications())) {
            d.setPublications(datasetVersion.getRelatedPublications().stream().map(p -> EBRAINSTranslatorUtils.getFormattedDigitalIdentifier(translatorUtils.getDoiCitationFormatter(), p.getIdentifier(), p.resolvedType())).filter(Objects::nonNull).map(Value::new).collect(Collectors.toList()));
        }
        ...
        if (!CollectionUtils.isEmpty(brainRegionOrNot.get(Boolean.TRUE))) {
            d.setStudiedBrainRegion(brainRegionOrNot.get(Boolean.TRUE).stream().map(EBRAINSTranslatorUtils::refAnatomical).collect(Collectors.toList()));
        }
        ...
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, datasetVersion.getInputResearchProductsFromInputFileBundles());
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, datasetVersion.getInputResearchProductsFromReverseOutputDOIs());
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, datasetVersion.getInputResearchProductsFromReverseOutputFiles());
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, datasetVersion.getInputResearchProductsFromReverseOutputFileBundles());
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, datasetVersion.getInputResearchProductsFromInputBrainAtlasVersions(), "Versions");
        ...
        Set<String> externalDOIs = EBRAINSTranslatorUtils.getExternalDOIs(datasetVersion.getInputDOIs());
        ...
        EBRAINSTranslatorUtils.addResearchProducts(outputResearchProducts, datasetVersion.getOutputResearchProductsFromReverseInputDOIs());
        EBRAINSTranslatorUtils.addResearchProducts(outputResearchProducts, datasetVersion.getOutputResearchProductsFromReverseInputFiles());
        EBRAINSTranslatorUtils.addResearchProducts(outputResearchProducts, datasetVersion.getOutputResearchProductsFromReverseInputFileBundles());
