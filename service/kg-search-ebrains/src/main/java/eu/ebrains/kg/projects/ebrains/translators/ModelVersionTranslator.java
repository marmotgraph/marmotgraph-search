...
import eu.ebrains.kg.projects.ebrains.source.ModelVersionV3;
import eu.ebrains.kg.projects.ebrains.source.commons.*;
import eu.ebrains.kg.projects.ebrains.target.ModelVersion;
...
public class ModelVersionTranslator extends EBRAINSTranslator<ModelVersionV3, ModelVersion, ModelVersionTranslator.Result> {

    public static class Result extends ResultsOfKG<ModelVersionV3> {
    }
    ...
    public ModelVersion translate(ModelVersionV3 modelVersion, DataStage dataStage, boolean liveMode, TranslatorUtils translatorUtils) throws TranslationException {
        ...
        if (accessibility == Accessibility.UNDER_EMBARGO) {
            m.setEmbargo(value(EBRAINSTranslatorUtils.createEmbargoMessage("model", modelVersion.getFileRepository(), null)));
        } else {
            if (modelVersion.getFileRepository() != null && modelVersion.getFileRepository().getIri() != null) {
                ...
                if (allowEmbedding) {
                    ...
                    if (modelVersion.getFileRepository() != null) {
                        if (EBRAINSTranslatorUtils.isCscsContainer(modelVersion.getFileRepository()) || EBRAINSTranslatorUtils.isDataProxyBucket(modelVersion.getFileRepository())) {
        ...
        if (!CollectionUtils.isEmpty(modelVersion.getRelatedPublications())) {
            m.setPublications(modelVersion.getRelatedPublications().stream().map(p -> EBRAINSTranslatorUtils.getFormattedDigitalIdentifier(translatorUtils.getDoiCitationFormatter(), p.getIdentifier(), p.resolvedType())).filter(Objects::nonNull).map(Value::new).collect(Collectors.toList()));
        }
        ...
        if (modelVersion.getModel() != null) {
            ...
            if(!CollectionUtils.isEmpty(brainStructureOrNot.get(Boolean.TRUE))){
                m.setBrainStructures(brainStructureOrNot.get(Boolean.TRUE).stream().map(EBRAINSTranslatorUtils::refAnatomical).collect(Collectors.toList()));
            }
        ...
        EBRAINSTranslatorUtils.addResearchProductsFromDOIs(inputResearchProducts, modelVersion.getInputDOIs());
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, modelVersion.getInputResearchProductsFromInputFiles());
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, modelVersion.getInputResearchProductsFromInputFileBundles());
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, modelVersion.getInputResearchProductsFromReverseOutputDOIs());
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, modelVersion.getInputResearchProductsFromReverseOutputFiles());
        EBRAINSTranslatorUtils.addResearchProducts(inputResearchProducts, modelVersion.getInputResearchProductsFromReverseOutputFileBundles());
        ...
        Set<String> externalDOIs = EBRAINSTranslatorUtils.getExternalDOIs(modelVersion.getInputDOIs());
        ...
        EBRAINSTranslatorUtils.addResearchProducts(outputResearchProducts, modelVersion.getOutputResearchProductsFromReverseInputDOIs());
        EBRAINSTranslatorUtils.addResearchProducts(outputResearchProducts, modelVersion.getOutputResearchProductsFromReverseInputFiles());
        EBRAINSTranslatorUtils.addResearchProducts(outputResearchProducts, modelVersion.getOutputResearchProductsFromReverseInputFileBundles());
        EBRAINSTranslatorUtils.addResearchProductsFromDOIs(outputResearchProducts, modelVersion.getOutputDOIs());
        EBRAINSTranslatorUtils.addResearchProducts(outputResearchProducts, modelVersion.getOutputResearchProductsFromOutputFiles());
        EBRAINSTranslatorUtils.addResearchProducts(outputResearchProducts, modelVersion.getOutputResearchProductsFromOutputFileBundles());
        ...
        Set<String> externalOutputDOIs = EBRAINSTranslatorUtils.getExternalDOIs(modelVersion.getOutputDOIs());
