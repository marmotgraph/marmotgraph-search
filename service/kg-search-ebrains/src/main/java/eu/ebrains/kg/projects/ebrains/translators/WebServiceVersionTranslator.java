...
public class WebServiceVersionTranslator extends EBRAINSTranslator<WebServiceVersionV3, WebServiceVersion, WebServiceVersionTranslator.Result> {
    ...
    public static class Result extends ResultsOfKG<WebServiceVersionV3> {
    }
    ...
    public List<String> semanticTypes() {
        return Collections.singletonList("https://openminds.om-i.org/types/WebServiceVersion");
    }
    ...
    public WebServiceVersion translate(WebServiceVersionV3 source, DataStage dataStage, boolean liveMode, TranslatorUtils translatorUtils) throws TranslationException {
        ...
        if(source.getCopyright()!=null){
            final String copyrightHolders = source.getCopyright().getHolder().stream().map(h -> EBRAINSTranslatorUtils.getFullName(h.getFullName(), h.getFamilyName(), h.getGivenName())).filter(Objects::nonNull).collect(Collectors.joining(", "));
        ...
        if(!CollectionUtils.isEmpty(source.getPublications())){
            w.setPublications(source.getPublications().stream().map(p -> EBRAINSTranslatorUtils.getFormattedDigitalIdentifier(translatorUtils.getDoiCitationFormatter(), p.getIdentifier(), p.resolvedType())).filter(Objects::nonNull).map(Value::new).collect(Collectors.toList()));
        }
