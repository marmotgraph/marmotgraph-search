...
import eu.ebrains.kg.common.controller.translation.models.TranslatorBase;
...
public class SpecimenTranslator extends TranslatorBase {
    ...
    public SpecimenTranslator(String datasetVersionId, List<String> errors, Map<String, Set<DOIReference>> specimenLookupMap) {
        this.translators = Arrays.asList(new SubjectTranslator(datasetVersionId, specimenLookupMap), new SubjectGroupTranslator(datasetVersionId, specimenLookupMap), new TissueSampleTranslator(datasetVersionId, specimenLookupMap), new TissueSampleCollectionTranslator(datasetVersionId, specimenLookupMap));
        this.errors = errors;
    }

    private final List<AbstractSpecimenTranslator<?>> translators;
    private final List<String> errors;

    private <T> AbstractSpecimenTranslator<T> getTranslator(DatasetVersionV3.StudiedSpecimen specimen) {
        return (AbstractSpecimenTranslator<T>) translators.stream().filter(t -> t.matches(specimen)).findFirst().orElse(null);
    }

    private <T> AbstractSpecimenTranslator<T> getTranslator(DatasetVersionV3.StudiedState state) {
        return (AbstractSpecimenTranslator<T>) translators.stream().filter(t -> t.matches(state)).findFirst().orElse(null);
    }
    ...
    public BasicHierarchyElement<DatasetVersion.DSVSpecimenOverview> translateToHierarchy(List<DatasetVersionV3.StudiedSpecimen> studiedSpecimen) {
        ...
        final SpecimenHierarchyTranslationContext translationContext = new SpecimenHierarchyTranslationContext(descendentStates, partOfRelations);
    ...
    private static String getFullStateLabel(AbstractSpecimenTranslator translator, String label, DatasetVersionV3.StudiedState state) {
    ...
    private static class SpecimenHierarchyTranslationContext {
        ...
        public SpecimenHierarchyTranslationContext(Map<String, List<DatasetVersionV3.StudiedState>> descendentStatesMap, Map<String, List<DatasetVersionV3.StudiedSpecimen>> partOfRelations) {
            this.descendentStatesMap = descendentStatesMap;
            this.partOfRelations = partOfRelations;
        }
    }

    private BasicHierarchyElement<Object> translateToBasicHierarchyElement(DatasetVersionV3.StudiedState studiedState, SpecimenHierarchyTranslationContext context, boolean attachRootElementAsChild, int order, String parentRelationType) {
        ...
        final AbstractSpecimenTranslator<?> translator = getTranslator(studiedState);
        if (translator != null) {
    ...
    private static abstract class AbstractSpecimenTranslator<T> extends TranslatorBase {
        ...
        private final Map<String, Set<DOIReference>> specimenLookupMap;

        public AbstractSpecimenTranslator(String prefix, String specimenType, String specimenColor, String stateType, String stateColor, String datasetVersionId, Map<String, Set<DOIReference>> specimenLookupMap) {
            this.prefix = prefix;
            ...
            this.specimenLookupMap = specimenLookupMap;
        }

        protected List<TargetInternalReference> getOtherPublications(UUID specimenId){
            final Set<DOIReference> doiReferences = specimenLookupMap.get(specimenId.toString());
            return doiReferences == null ? null : doiReferences.stream().map(doiReference ->
                    new TargetInternalReference(
                            doiReference.getUuid().toString(),
                            EBRAINSTranslatorUtils.stripDOIPrefix(doiReference.getDoi()),
                            new TargetInternalReference.Context(
                                    "Specimens",
                                    specimenId.toString())
                    )
            ).sorted(Comparator.comparing(TargetInternalReference::getValue)).toList();
        }
    ...
    private static class SubjectTranslator extends AbstractSpecimenTranslator<DatasetVersion.DSVSubject> {
        ...
        public DatasetVersion.DSVSubject translateSpecimen(DatasetVersionV3.StudiedSpecimen specimen, DatasetVersionV3.StudiedSpecimen parentSpecimen) {
            ...
            final String uuid = IdUtils.getUUID(specimen.getId());
            sub.setId(uuid);
            if(uuid!=null) {
                sub.setOtherPublications(getOtherPublications(UUID.fromString(uuid)));
            }
    ...
    private static class SubjectGroupTranslator extends AbstractSpecimenTranslator<DatasetVersion.DSVSubjectGroup> {
        ...
        public DatasetVersion.DSVSubjectGroup translateSpecimen(DatasetVersionV3.StudiedSpecimen specimen, DatasetVersionV3.StudiedSpecimen parentSpecimen) {
            ...
            if(uuid!=null) {
                subGrp.setOtherPublications(getOtherPublications(UUID.fromString(uuid)));
            }
    ...
    private static class TissueSampleTranslator extends AbstractSpecimenTranslator<DatasetVersion.DSVTissueSample> {
        ...
        public DatasetVersion.DSVTissueSample translateSpecimen(DatasetVersionV3.StudiedSpecimen specimen, DatasetVersionV3.StudiedSpecimen parentSpecimen) {
            ...
            final String uuid = IdUtils.getUUID(specimen.getId());
            tissueSample.setId(uuid);
            tissueSample.setTitle(value(specimen.getInternalIdentifier()));
//            if(uuid!=null) {
//                tissueSample.setOtherPublications(getOtherPublications(UUID.fromString(uuid)));
//            }
        ...
        private void fillTissueSampleInformation(DatasetVersion.DSVTissueSample tissueSample, DatasetVersionV3.StudiedSpecimen specimen) {
            ...
            tissueSample.setAnatomicalLocation(EBRAINSTranslatorUtils.refAnatomical(specimen.getAnatomicalLocation(), true));
    ...
    private static class TissueSampleCollectionTranslator extends AbstractSpecimenTranslator<DatasetVersion.DSVTissueSampleCollection> {
        ...
        public DatasetVersion.DSVTissueSampleCollection translateSpecimen(DatasetVersionV3.StudiedSpecimen specimen, DatasetVersionV3.StudiedSpecimen parentSpecimen) {
            ...
            final String uuid = IdUtils.getUUID(specimen.getId());
            tissueSampleColl.setId(uuid);
            tissueSampleColl.setTitle(value(specimen.getInternalIdentifier()));
//            if(uuid!=null) {
//                tissueSampleColl.setOtherPublications(getOtherPublications(UUID.fromString(uuid)));
//            }
        ...
        private void fillTissueSampleCollectionInformation(DatasetVersion.DSVTissueSampleCollection tissueSampleCollection, DatasetVersionV3.StudiedSpecimen specimen) {
            ...
            tissueSampleCollection.setAnatomicalLocation(EBRAINSTranslatorUtils.refAnatomical(specimen.getAnatomicalLocation(), true));
    ...
    private <T> BasicHierarchyElement<Object> translateToBasicHierarchyElement(DatasetVersionV3.StudiedSpecimen studiedSpecimen, SpecimenHierarchyTranslationContext context, boolean attachRootElementAsChild, String parentRelationType, DatasetVersionV3.StudiedSpecimen parentSpecimen) {
        ...
        final AbstractSpecimenTranslator<T> specimenTranslator = getTranslator(studiedSpecimen);
        T data = null;
    ...
    private String findLabelForColor(String color) {
        if (color != null) {
            final AbstractSpecimenTranslator<?> translatorByColor = translators.stream().filter(f -> f.stateColor.equals(color) || f.specimenColor.equals(color)).findFirst().orElse(null);
