...
public class EBRAINSTranslatorUtils {
    private final static Logger logger = LoggerFactory.getLogger(EBRAINSTranslatorUtils.class);
    ...
    public static List<TargetInternalReference> refAnatomical(List<AnatomicalLocation> anatomicalLocations, boolean sorted) {
        if (!CollectionUtils.isEmpty(anatomicalLocations)) {
            Stream<TargetInternalReference> targetInternalReferenceStream = anatomicalLocations.stream().map(EBRAINSTranslatorUtils::refAnatomical).filter(Objects::nonNull);
            if (sorted) {
